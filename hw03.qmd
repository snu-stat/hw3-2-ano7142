...
# 추가로 필요한 패키지 설치
# - scipy        : curve_fit(문제 2-1.1), 카이제곱 분포 (진단)
# - statsmodels  : GLM (Binomial / Poisson / NegativeBinomial)
# - patsy        : statsmodels formula API
# - notebook/ipykernel/nbformat : Binder용 .ipynb 실행 환경
#   ("jupyter" 메타패키지 대신 핵심만 명시: 솔버 부담↓)

# 5. R 패키지 설치 (reticulate 및 필수 패키지)
RUN R -e "install.packages(c('reticulate', 'remotes', 'IRkernel'))" && \
    R -e "IRkernel::installspec(user = FALSE)"
# 추가로 필요한 R 패키지 설치
# - Lahman : Teams 데이터 (2-1 ~ 2-4)
# - NHANES : 흡연 자료 (1-1)
# - broom, MASS : tidyverse 이미지 포함이지만 의존성 명시
...
가이드 템플릿에 비해 다음과 같은 부분만 추가/보강하였다.
